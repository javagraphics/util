 ...
 *
 * $Date: 2015-09-21 07:23:00 +0200 (Mo, 21 Sep 2015) $
 *
 ...
 * <p>Eventually we might want to ramp this up to rival
 * the {@link ObservableList} in terms of synchronized modification safeguards and
 * <code>RecursiveListenerModificationExceptions</code>.
 * <p>Notes this includes parameterized keys, and it is technically possible to have
 * two keys that use the same name but map to different types.
 */
public class ObservableProperties implements Serializable {
	 ...
	 * <p>This is needed when it is dangerous for a listener to get
	 * an incomplete picture of the state of this object. For example
	 * if you are changing the VALUE key to 5 and the MAXIMUM key to 8,
	 * if a listener were notified after the first step and the MAXIMUM value
	 * was an older value (such as 3), then it might throw an exception
	 * because it assumes the VALUE is always less than the MAXIMUM.
	...
	/** This must be called after {@link #beginEdit()} to reactive
	 * listeners for this object.
	 * @param endedEdit a token created by a call to {@link #beginEdit()}. When
	 * there are no unresolved edits: all pending PropertyChangeEvents will be flushed.
	 */
	...
	private static class QueueElement {
		...
		QueueElement(PropertyGroup group,Key<?> key,Object oldValue,Object newValue) {
			this.group = group;
			this.key = key;
		...
		synchronized(eventQueue) {
			while(eventQueue.size()>0) {
				...
				synchronized(q.group.listeners) {
					listenerArray = q.group.listeners.toArray(new Listener[q.group.listeners.size()]);
				}
				
				// TODO: consolidate QueueElements so we only include the latest change
				for(Listener l : listenerArray) {
					boolean keysMatch = l.key==null || l.key.equals(q.key);
					if(keysMatch) {
						try {
							l.pcl.propertyChange(new PropertyChangeEvent(ObservableProperties.this, q.key.keyName, q.oldValue, q.newValue));
						} catch(Exception e) {
							e.printStackTrace();
						}
					}
	...
	private static class ChangeListenerWrapper implements PropertyChangeListener {
	...
	private static class ActionListenerWrapper implements PropertyChangeListener {
		...
		public void propertyChange(PropertyChangeEvent evt) {
			actionListener.actionPerformed(new ActionEvent(evt.getSource(), 0, evt.getPropertyName()));
		}
	}
	...
	public static class NonNullBoundsChecker extends BoundsChecker implements Serializable {
		...
		public void check(Object t,Key key) throws IllegalArgumentException {
			if(t==null) throw new NullPointerException(key.getKeyName()+" cannot be null");
		}
	}
	...
	public static class NumberBoundsChecker extends BoundsChecker<Number> {
		 ...
		 * @param includeMin if true then this applies a "greater than or equal to" check.
		 * If false then this simply applies a "greater than" check, so if someone tries to
		 * use a value that is exactly equal to the minimum: it will fail.
		 * @param includeMax if true then this applies a "less than or equal to" check.
		 * If false then this simply applies a "less than" check, so if someone tries to
		 * use a value that is exactly equal to the maximum: it will fail.
		 */
		public NumberBoundsChecker(Number min, Number max, boolean includeMin, boolean includeMax) {
			this.min = min;
			this.max = max;
		...
		@Override
		public void check(Number t,Key<Number> key) throws IllegalArgumentException {
			boolean bad = ( includeMin && t.doubleValue()<min.doubleValue() ) ||
					...
					( (!includeMax) && t.doubleValue()>=max.doubleValue() );
			if(bad)
				throw new IllegalArgumentException("the value \""+key.toString()+"\" ("+t+") must be within "+toString());
		}
	...
	/** A key for a key/value pair. The reason keys are not
	 * simply <code>Strings</code> is that these objects
	 * have a parameterized type: so casting is not necessary.
	 * <p>You can technically have two keys with the same name
	 * and different types in the same property set.
	 * <p>It is recommended (but not necessary) that keys be stored as public static fields.
	 *
	 * @param <T> the type of value this key should map to.
	 */
	public static class Key<T> implements Serializable, Comparable<Key<?>> {
		 ...
		 * @param type the parameterized type of this key.
		 * (Is there a way to ascertain this by reflection? Stating this here seems
		 ...
		 * (Is there a way to ascertain this by reflection? Stating this here seems
		 * redundant, but necessary.)
		 * @param checker an optional {@link BoundsChecker} to throw an exception
		 * if someone attempts to assign a value that isn't allowed.
		 */
		 ...
		 * @param defaultValue the optional default value if the map doesn't contain the key.
		 * @return the value of the key provided, or the default value if no value is identified.
		 */
		 ...
		 * use a value that is exactly equal to the maximum: it will fail.
		 */
		public Key(String keyName,Class<T> type,Number min,Number max,boolean includeMin,boolean includeMax) {
			this(keyName, type, new NumberBoundsChecker(min, max, includeMin, includeMax));
		}

		/** @return true if a PropertyChangeEvent relates to this Key. 
		 * @param evt the evt to check against.
		 */
		public boolean matches(PropertyChangeEvent evt) {
			return evt.getPropertyName().equals(keyName) || evt.getPropertyName().endsWith("#"+keyName);
		}
		...
		public int compareTo(Key<?> o) {
			...
			if(c==0) {
				...
				if(type.isAssignableFrom(o.type) || o.type.isAssignableFrom(type)) {
					//this is OK
				} else {
	...
	private class PropertyGroup implements Serializable {
		...
		boolean addPropertyChangeListener(Key<?> key,PropertyChangeListener pcl,Object identifier) {
			if(listeners==null)
				listeners = new ArrayList<Listener>();
			synchronized(listeners) {
				for(int a = 0; a<listeners.size(); a++) {
					Listener l = listeners.get(a);
					if(l.id==identifier && l.pcl==pcl && equals(l.key, key)) {
						return false;
					} else if(l.id==identifier && equals(l.key, key)) {
						throw new IllegalArgumentException("This identifier is already declared: "+identifier);
					}
				}
		...
		void fireListeners(Key<?> key, Object oldValue, Object newValue) {
			if( equals(oldValue, newValue) || listeners==null) return;
		...
		/** 
		 * Copied from Objects.equals() for Java 1.6 compatibility:
	...
	/** The string "default". This is recommended for a group of properties that represent
	 * vital elements of an object, and are serializable.
	 */
	public static final String DEFAULT = "default";

	/** The string "transient". This is recommended for a group of properties that are not
	 * meant to be serialized (cached indices, renderings, etc.).
	 */
	...
	public ObservableProperties(ObservableProperties p) {
		this(p.possibleGroupNames==null ? (String[])null : p.possibleGroupNames.toArray(new String[p.possibleGroupNames.size()]));
	 ...
	 * @param groupNames the group IDs this object allows. (If null: then all group IDs are allowed.)
	 * If non-null: then attempting to access a group ID not in this set will result in an exception.
	 */
	 ...
	 * Strings passed during construction that limit the possible group names.)
	 */
	 ...
	 * @param propertyGroup the group of properties this relates to.
	 * For example: {@link #DEFAULT} or {@link #TRANSIENT}.
	 * @param pcl the listener to add.
	 */
	public synchronized void addListener(String propertyGroup,PropertyChangeListener pcl) {
	 ...
	 * @param propertyGroup the group of properties this relates to.
	 * For example: {@link #DEFAULT} or {@link #TRANSIENT}.
	 * @param key an optional key that limits what this listener which notified about.
	 * If null: then this listener will be notified for all changes.
	 * @param pcl the listener to add.
	 */
	public synchronized void addListener(String propertyGroup,Key<?> key,PropertyChangeListener pcl) {
	 ...
	 * @param key an optional key that limits what this listener which notified about.
	 * If null: then this listener will be notified for all changes.
	 * @param changeListener the ChangeListener to be notified
	 */
	public synchronized void addListener(String propertyGroup,Key<?> key,ChangeListener changeListener) {
		PropertyGroup group = getGroup(propertyGroup);
		group.addPropertyChangeListener(key, new ChangeListenerWrapper(changeListener), changeListener);
	}
	 ...
	 * @param key an optional key that limits what this listener which notified about.
	 * If null: then this listener will be notified for all changes.
	 * @param actionListener the ActionListener to be notified.
	 */
	public synchronized void addListener(String propertyGroup,Key<?> key,ActionListener actionListener) {
		PropertyGroup group = getGroup(propertyGroup);
		group.addPropertyChangeListener(key, new ActionListenerWrapper(actionListener), actionListener);
	}

	/** Return all the PropertyChangeListeners that listen for changes to
	 * DEFAULT properties.
	 */
	...
	/** Return all the PropertyChangeListeners that listen for changes to
	 * a given group of properties.
	 * @param propertyGroup the group of properties this relates to.
	 * For example: {@link #DEFAULT} or {@link #TRANSIENT}.
	 */
	public synchronized PropertyChangeListener[] getPropertyListeners(String propertyGroup) {
		PropertyGroup group = getGroup(propertyGroup);
		if(group.listeners==null) return new PropertyChangeListener[] {};
	 ...
	 * @param propertyGroup the group this listener belongs to.
	 * @param key the key this listener relates to.
	 * @param changeListener the listener to remove.
	 */
	public synchronized void removeListener(String propertyGroup,Key<?> key,ChangeListener changeListener) {
		PropertyGroup group = getGroup(propertyGroup);
	 ...
	 */
	public synchronized void removeListener(String propertyGroup,Key<?> key,ActionListener actionListener) {
		PropertyGroup group = getGroup(propertyGroup);
	 ...
	 * @param pcl the listener to remove.
	 */
	public synchronized void removeListener(String propertyGroup,PropertyChangeListener pcl) {
	 ...
	 * @param pcl the listener to remove.
	  */
	public synchronized void removeListener(String propertyGroup,Key<?> key,PropertyChangeListener pcl) {
		PropertyGroup group = getGroup(propertyGroup);
	 ...
	 * 
	 * @param propertyGroup the property group to consult. (Recommended values are DEFAULT or TRANSIENT.) This
	 * cannot be null. Each ObservableProperties object may be constructed with a finite set of acceptable
	 * property groups: if that is the case for this object and this group ID is not part of this set, then
	 * an exception is thrown.
	 * @param key the key to set.
	 * @param value the value to set. If null: then this key is removed.
	 ...
	 * This will include all group IDs and prepend them to keys.
	 */
	 ...
	 * @param propertyGroup the group of keys to return.
	 */
	...
	/** @return a condensed map of all group IDs.
	 * @param prependGroupID if true, then the groupID will be prepended to all keys.
	 * It is highly recommended that this be true, because if this is false and different
	 * groups have similar sounding keys: then those groups cannot be safely condensed
	 * to a single map and exception will be thrown.
	 * @param identifyClassNames if true then keys will be uniquely identified by
	 * including their class name.
	 * @param groupIDs an optional list of group IDs to poll. If null: then all group IDs
	 * are included. For example: you might want to only return the DEFAULT group, at
	 * which point it would be safe to pass <code>false</code> for <code>prependGroupID</code>.
	 * 
	 **/
	public synchronized Map<String, Object> getMap(boolean prependGroupID, boolean identifyClassNames,String... groupIDs) {
				...
				for(Key<?> key : group.map.keySet()) {
					if(prependGroupID) {
						if(identifyClassNames) {
							if(key.type!=null) {
								returnValue.put( groupID+"-"+key.type.getName()+"#"+key.toString(), group.map.get(key) );
							} else {
								returnValue.put( groupID+"#"+key.toString(), group.map.get(key) );
							}
						} else {
							Object oldValue = returnValue.put( groupID+"#"+key.toString(), group.map.get(key) );
							if(oldValue!=null) {
						...
						if(identifyClassNames) {
							Object oldValue = returnValue.put( key.type.getName()+"-"+key.toString(), group.map.get(key) );
							if(oldValue!=null) {
							...
							Object oldValue = returnValue.put( key.toString(), group.map.get(key) );
							if(oldValue!=null) {
	...
	/** Return a PropertyGroup. This will create one if necessary, or throw an exception if this ID
	 * is not allowed.
	 */
	private synchronized PropertyGroup getGroup(String groupID) {
		...
		if(group==null) {
			...
			if(possibleGroupNames!=null && (!possibleGroupNames.contains(groupID))) {
				throw new IllegalArgumentException("the group \""+groupID+"\" is not included in this ObservableProperties.");
			}
	 ...
	 * 
	 * @param groupName the name of the group (such as DEFAULT or TRANSIENT)
	 * @return all the properties in the group provided. This will be an empty map if the group name
	 * is unused.
	 */
